...
fn get_result(file_name: &str) -> i32 {
    ...
    let mut line_to_numbers_map: HashMap<usize, Vec<NextNumber>> = HashMap::new();
    let mut line_to_gears_map: HashMap<usize, Vec<usize>> = HashMap::new();
    ...
    for i in 0..number_of_lines {
        ...
        let mut numbers: Vec<NextNumber> = vec![];

        begin_at_index = 0;

        for (index, c) in curr_line.unwrap().char_indices() {
            if c == '*' {
                gears.push(index);
            }

            if c.is_numeric() {
                let next_number = find_next_number(curr_line.unwrap(), index);
                numbers.push(next_number.unwrap());
            }
        }

        line_to_gears_map.insert(i, gears);
        line_to_numbers_map.insert(i, numbers);
    }
